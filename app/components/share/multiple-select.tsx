...
import { ChevronDownIcon, ChevronUpIcon, Loader2, Search, X } from 'lucide-react';
...
export const MultipleSelect = ({
  ...
  const handleCheckboxChange = (itemValue: string, checked: boolean) => {
    ...
    if (checked) {
      ...
      newValues = currentValues.includes(itemValue) ? currentValues : [...currentValues, itemValue];
    } else {
      // If isNullValue is false, don't allow unchecking the last item
      if (!isNullValue && currentValues.length === 1 && currentValues.includes(itemValue)) {
        return;
      }
          ...
          <button
            ...
            <div
              ...
              <div className="flex flex-1 flex-wrap items-center gap-1 overflow-hidden">
                {getDisplayValue() || <span className="text-muted-foreground">{placeholder}</span>}
              </div>
              ...
              <div className="flex shrink-0 items-center gap-2">
                {isLoading ? (
                  ...
                  <ChevronUpIcon className="h-4 w-4 shrink-0 opacity-50" color="#71717A" />
                ) : (
                  <ChevronDownIcon className="h-4 w-4 shrink-0 opacity-50" color="#71717A" />
                )}
              </div>
            </div>
          </button>
        ...
        <PopoverContent
          ...
          <div className="max-h-60 space-y-2 overflow-y-auto p-3">
            {filteredOptions.map((option, index) => {
              ...
              return (
                <div key={`${itemValue}-${index}`} className="flex items-center gap-2">
                  <Checkbox
                    checked={isChecked}
                    disabled={isDisabled}
                    id={name ? `${name}-${itemValue}` : itemValue}
                    onCheckedChange={(checked) => handleCheckboxChange(itemValue, checked === true)}
                  />
                  <Label
                    className={cn(
          ...
          {showSelectAllActions && (
            <div className="flex justify-between border-t p-2">
              <Button variant="ghost" size="sm" onClick={handleDeselectAll} disabled={!isNullValue}>
                {deselectAllLabel}
              </Button>
