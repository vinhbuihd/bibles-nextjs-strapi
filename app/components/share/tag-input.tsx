  ...
  return (
    <div
      ...
      {value.map((tag, index) => (
        <span
          key={`${tag}-${index}`}
          className={cn('bg-muted flex items-center gap-1 rounded px-2 py-0.5 hover:bg-[#F4F4F5]', {
          ...
          {!disabled && (
            <button
              ...
              <X size={12} className="text-muted-foreground hover:text-foreground cursor-pointer" />
            </button>
          )}
        </span>
      ))}
